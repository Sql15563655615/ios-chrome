...
| Printing | `chromium/src/BUILD.gn`; `chromium/src/printing/BUILD.gn` | Root `//BUILD.gn` contains printing tests and PPAPI printing examples in broad non-iOS target sets. | Minimal args set `enable_basic_printing=false` and `enable_print_preview=false`; iOS target pruning should avoid most printing targets. |
